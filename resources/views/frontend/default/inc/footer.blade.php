...
<footer class="gshop-footer position-relative pt-8 bg-dark z-1 overflow-hidden">
  <div class="main-footer__bg z--1"
    data-background="{{ staticAsset('frontend/freshmeat/assets/images/backgrounds/footer-bg.png') }}">
  </div>
      ...
      <div class="col-xl-5 col-lg-6">
        <div class="gshop_subscribe_form text-center">
          <h4 class="text-white gshop-title">{{ localize('Subscribe to the us') }}<mark
              ...
              <img src="{{ staticAsset('frontend/default/assets/img/shapes/border-line.svg') }}" alt="border line"
                class="position-absolute border-line"></mark><br
          ...
          <form class="mt-5 d-flex align-items-center bg-white rounded subscribe_form"
            ...
            <button type="submit" class="btn btn-primary flex-shrink-0">{{ localize('Subscribe Now') }}</button>
          </form>
        </div>
      </div>
    </div>
        ...
        <div class="footer-widget">
          ...
          <ul class="footer-nav">
            @foreach ($pages as $page)
              <li><a href="{{ route('home.pages.show', $page->slug) }}">{{ $page->collectLocalization('title') }}</a>
              </li>
            @endforeach
          </ul>
        </div>
      ...
      <div class="row align-items-center g-3">
        ...
        <div class="col-lg-4 d-none d-lg-block">
          <div class="logo-wrapper text-center">
            <a href="{{ route('home') }}" class="logo"><img src="{{ uploadedAsset(getSetting('footer_logo')) }}"
                alt="footer logo" class="img-fluid" width="100"></a>
          </div>
        </div>
        <div class="col-lg-4">
